...
export default function SpecialOffers() {
  return (
    <section id="offers" className="py-24 bg-transparent">
          ...
          {offersData.offers.map((pkg, index) => (
            <div 
              ...
              className={`${pkg.highlighted 
                ...
                : 'bg-[#DEDEDE]'} rounded-3xl p-8 hover:shadow-xl transition-all duration-300 hover:-translate-y-2 relative overflow-hidden`}
            >
              {pkg.highlighted && (
              ...
              <div className={`mb-6 ${pkg.highlighted ? 'relative z-10' : ''}`}>
                ...
                <h3 className={`text-2xl font-semibold mb-3 ${pkg.highlighted ? 'text-white' : 'text-[#171717]'}`}>
                  {pkg.title}
                </h3>
                <p className={pkg.highlighted ? 'text-orange-100' : 'text-[#171717]'}>
                  {pkg.description}
                </p>
              </div>
              <div className={`space-y-3 mb-8 ${pkg.highlighted ? 'relative z-10' : ''}`}>
                {pkg.features.map((feature, featureIndex) => (
                  <div key={featureIndex} className={`flex items-center ${pkg.highlighted ? 'text-orange-100' : 'text-[#171717]'}`}>
              ...
              <div className={`mb-6 ${pkg.highlighted ? 'relative z-10' : ''}`}>
                <span className={`text-4xl font-bold ${pkg.highlighted ? 'text-white' : 'text-[#171717]'}`}>
                  {pkg.price}
                </span>
              </div>
              <button className={`w-full py-3 rounded-full font-medium transition-all duration-300 ${
                pkg.highlighted 
                  ...
                  : pkg.title === 'Premium' 
                    ...
                    : 'bg-white text-[#171717] border border-gray-200 hover:bg-orange-500 hover:text-white hover:border-orange-500'
              }`}>
                {pkg.buttonText}
              </button>
            </div>
