export const heroContent = {
  title: "Premium",
  titleAccent: "Photography",
  titleSuffix: "for Brands",
  subtitle: "Creating visual stories that elevate your brand through stunning product and commercial photography",
  ctaButton: "Discuss Your Project"
}

export const aboutContent = {
  title: "About",
  titleAccent: "My Vision",
  description: [
    "With over 7 years of experience in commercial photography, I specialize in creating visual narratives that elevate brands and capture the essence of their products.",
    "My approach combines technical precision with artistic vision, delivering images that not only look stunning but also resonate with your target audience and drive business results.",
    "From startups to established luxury brands, I craft photography that tells your unique story and sets you apart in the marketplace."
  ],
  stats: {
    projects: "500+",
    projectsLabel: "Projects",
    years: "7+",
    yearsLabel: "Years",
    brands: "50+",
    brandsLabel: "Brands"
  }
}

export const offersContent = {
  title: "Special",
  titleAccent: "Offers",
  subtitle: "Exclusive packages designed to elevate your brand photography",
  packages: [
    {
      title: "Essential",
      description: "Perfect for emerging brands",
      features: [
        "15 product photos",
        "Professional retouching",
        "3-day delivery"
      ],
      price: "$599",
      button: "Get Started"
    },
    {
      title: "Professional",
      description: "Most popular choice",
      features: [
        "35 product photos",
        "Premium retouching",
        "Lifestyle shots",
        "2-day delivery"
      ],
      price: "$1299",
      button: "Choose Professional",
      featured: true
    },
    {
      title: "Premium",
      description: "Complete brand solution",
      features: [
        "60+ product photos",
        "Ultra-premium retouching",
        "Creative direction",
        "Video content included",
        "Priority delivery"
      ],
      price: "$2499",
      button: "Go Premium"
    }
  ]
}

export const servicesContent = {
  title: "Преміум",
  titleAccent: "послуги",
  subtitle: "Спеціалізовані фоторішення, розроблені з урахуванням унікальних потреб вашого бренду",
  services: [
    {
      title: "Product Photography",
      description: "Studio-quality shots that showcase your products in the best light",
      features: [
        "Professional lighting",
        "Multiple angles",
        "Detail focus",
        "Brand consistency"
      ]
    },
    {
      title: "Brand Photography",
      description: "Complete visual identity creation for your brand story",
      features: [
        "Creative direction",
        "Campaign concepts",
        "Style guides",
        "Visual storytelling"
      ]
    },
    {
      title: "E-commerce Solutions",
      description: "Optimized imagery for online retail and marketplaces",
      features: [
        "White background",
        "360° views",
        "Size reference",
        "Platform ready"
      ]
    },
    {
      title: "Food & Lifestyle",
      description: "Appetizing visuals that capture taste and atmosphere",
      features: [
        "Food styling",
        "Natural lighting",
        "Texture focus",
        "Environmental shots"
      ]
    }
  ]
}

export const portfolioContent = {
  title: "Портфоліо",
  titleAccent: "",
  subtitle: "Мої останні роботи та проєкти",
  items: [
    {
      category: "Product Photography",
      title: "Luxury Watch Collection",
      description: "Elegant timepieces captured with precision lighting",
      featured: true
    },
    {
      category: "Food Photography",
      title: "Artisan Bakery",
      description: "Fresh breads and pastries showcasing craftsmanship",
      featured: false
    },
    {
      category: "Brand Photography",
      title: "Fashion Brand Campaign",
      description: "Complete visual identity for emerging fashion label",
      featured: false
    },
    {
      category: "E-commerce",
      title: "Skincare Products",
      description: "Clean, minimalist shots for online retail",
      featured: false
    },
    {
      category: "Lifestyle",
      title: "Coffee Brand Story",
      description: "Authentic moments and product integration",
      featured: true
    },
    {
      category: "Product Photography",
      title: "Jewelry Collection",
      description: "Detailed shots highlighting fine craftsmanship",
      featured: false
    }
  ],
  button: ""
}

export const processContent = {
  title: "Work",
  titleAccent: "Process",
  subtitle: "A streamlined approach to deliver exceptional results for your brand",
  steps: [
    {
      step: "01",
      title: "Discovery",
      description: "We explore your brand vision, goals, and photography requirements to establish creative direction."
    },
    {
      step: "02",
      title: "Strategy",
      description: "Comprehensive planning including shot lists, styling concepts, and scheduling for seamless execution."
    },
    {
      step: "03",
      title: "Creation",
      description: "Professional photography session with meticulous attention to lighting, composition, and brand essence."
    },
    {
      step: "04",
      title: "Refinement",
      description: "Expert retouching and editing to enhance images while preserving authenticity and brand consistency."
    },
    {
      step: "05",
      title: "Delivery",
      description: "Timely delivery of optimized high-resolution images ready for all your marketing channels."
    }
  ],
  conditions: {
    title: "Working",
    titleAccent: "Conditions",
    payment: {
      title: "Payment Terms",
      items: [
        "50% deposit required to secure booking",
        "Remaining 50% due upon project completion",
        "Payment via bank transfer or PayPal",
        "Detailed invoice provided"
      ]
    },
    timeline: {
      title: "Timeline & Delivery",
      items: [
        "Standard delivery: 5-7 business days",
        "Express delivery available (additional fee)",
        "2 rounds of revisions included",
        "Secure cloud delivery of final files"
      ]
    }
  }
}

export const contactsContent = {
  title: "Get In",
  titleAccent: "Touch",
  subtitle: "Ready to create stunning visuals for your brand? Let's discuss your project",
  contactDetails: {
    title: "Contact",
    titleAccent: "Details",
    items: [
      {
        label: "Email",
        value: "hello@photography.com"
      },
      {
        label: "Phone",
        value: "+1 (555) 123-4567"
      },
      {
        label: "Studio Location",
        value: "123 Creative Street, Art District, NY 10001"
      },
      {
        label: "Business Hours",
        value: "Monday - Friday: 9:00 AM - 6:00 PM",
        value2: "Saturday: 10:00 AM - 4:00 PM"
      }
    ]
  },
  social: {
    title: "Follow Me"
  },
  form: {
    title: "Send a",
    titleAccent: "Message",
    fields: {
      name: "Your name",
      email: "your@email.com",
      project: "Select a project type",
      message: "Tell me about your project..."
    },
    projectTypes: [
      { value: "product", label: "Product Photography" },
      { value: "brand", label: "Brand Photography" },
      { value: "ecommerce", label: "E-commerce" },
      { value: "food", label: "Food & Beverage" },
      { value: "other", label: "Other" }
    ],
    button: "Send Message"
  }
}

export const footerContent = {
  title: "Photo",
  titleAccent: "graphy",
  description: "Professional brand and product photography services for businesses that value quality and creativity.",
  services: [
    "Product Photography",
    "Brand Photography",
    "E-commerce Solutions",
    "Food & Beverage"
  ],
  quickLinks: [
    "About",
    "Portfolio",
    "Process",
    "Contact"
  ],
  contact: [
    "hello@photography.com",
    "+1 (555) 123-4567",
    "New York, NY"
  ],
  legal: {
    copyright: "© 2024 Photography Studio. All rights reserved.",
    privacy: "Privacy Policy",
    terms: "Terms of Service"
  }
}
